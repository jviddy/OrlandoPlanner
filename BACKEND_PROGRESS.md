...
- [~] Google OAuth route implementation (`/api/auth/google/start` and callback) — source and local Cloudflare preview verified; production verification is blocked because the current Pages project reports `Git Provider: No` and cannot be converted in place; create a new Git-integrated Pages project and cut over safely (see `docs/cloudflare-pages-git-worker-migration.md`)
