...
const { isServerBacked, uploadCurrentTrip } = useServerTrip()
...
async function prepareShare() {
  ...
  try {
    ...
    prepareMessage.value = err?.message || 'Could not save a shareable copy. Please try again.'
    return false
  } finally {
...
async function open() {
  ...
  includeTripName.value = false
  includeSafeDetails.value = false
  selectedDayIds.value = store.days.filter((day) => day.parkId).slice(0, 2).map((day) => day.id)
  await prepareShare()
  nextTick(() => { document.querySelector<HTMLElement>('.share-studio>header button')?.focus(); generate() })
}
  ...
  <Teleport to="body">
    <div v-if="isOpen" class="share-root">
      ...
      <section class="share-studio" role="dialog" aria-modal="true" aria-labelledby="share-title">
        ...
        <div v-if="preparing" class="share-studio__preparing">
          <p>{{ prepareMessage || 'Saving a shareable copy…' }}</p>
        </div>
        <div v-else class="share-studio__body">
        ...
        <footer><button type="button" class="ghost" @click="close">Close</button><button type="button" class="cta" :disabled="preparing || !blobs.length || generating" @click="shareOrDownload">Share / download {{ blobs.length || '' }}</button></footer>
      </section>
    </div>
