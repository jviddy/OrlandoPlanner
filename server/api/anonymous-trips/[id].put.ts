import { bearer, requireAnonymousDb, tokenHash, validateTripPayload } from '../../utils/anonymousTrips'

export default defineEventHandler(async (event) => {
  const db = requireAnonymousDb(event); const id = getRouterParam(event, 'id') ?? ''; const expected = Number(getHeader(event, 'if-match'))
  ...
  const result = await db.prepare('UPDATE anonymous_trips SET payload = ?, revision = revision + 1, updated_at = ? WHERE id = ? AND edit_token_hash = ? AND revision = ? AND deleted_at IS NULL').bind(JSON.stringify({ ...body, tripId: id }), now, id, hash, expected).run()
