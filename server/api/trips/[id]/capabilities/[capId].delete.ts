...
export default defineEventHandler(async (event) => {
  ...
  const capId = getRouterParam(event, 'capId') ?? ''
  ...
  const result = await db.prepare('UPDATE trip_capabilities SET revoked_at = ? WHERE id = ? AND trip_id = ? AND revoked_at IS NULL').bind(now, capId, tripId).run()
  if (!result.meta?.changes) throw createError({ statusCode: 404, statusMessage: 'Capability not found', data: { code: 'capability_not_found' } })
