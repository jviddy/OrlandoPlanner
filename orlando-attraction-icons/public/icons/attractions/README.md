# OrlandoPlanner attraction icons
...
From the ZIP root, copy the included `public/` folder into your repository root, merging with your existing public directory. The resulting location is `public/icons/attractions/`. No build step, runtime library or network request to a third party is required.
...
Use `alt=""` when a visible attraction name accompanies the icon. For a standalone meaningful image, set `alt` to the attraction name. An icon-only button needs an accessible name on the button. These abstract motifs indicate activity, so keep visible names for reliable attraction identification.
...
- Generic geometric activity motifs; no traced logos, characters, branded castle profiles, themed volcano profiles, signature architecture or branded toy studs.
